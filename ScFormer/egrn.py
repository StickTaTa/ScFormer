...
def peak_Sparse(peak_feature, gene_feature, device):
    ...
    for peak_num in range(peak_feature.shape[0]):
        ...
        peak_combine_row[non_count:non_count + peak_sum * gene_feature.shape[0]] = (
                peak_row[peak_row == peak_num] + peak_init - peak_num).reshape(1, -1)
        peak_combine_col[non_count:non_count + peak_sum * gene_feature.shape[0]] = (
            peak_col[peak_row == peak_num]).repeat(gene_feature.shape[0])
        peak_combine_data[non_count:non_count + peak_sum * gene_feature.shape[0]] = (
            peak_data[peak_row == peak_num]).repeat(gene_feature.shape[0])

        non_count += peak_sum * gene_feature.shape[0]

    # Create the final sparse peak_feature tensor
    peak_feature_ori = torch.sparse.FloatTensor(torch.vstack((peak_combine_row, peak_combine_col)).long(),
                                                peak_combine_data, torch.Size(
            (gene_feature.shape[0] * peak_feature.shape[0], peak_feature.shape[1])))
...
def gene_Sparse(peak_feature, gene_feature, device):
    ...
    gene_feature_ori = torch.sparse.FloatTensor(torch.vstack((gene_combine_row, gene_combine_col)), gene_combine_data,
                                                torch.Size((gene_feature.shape[0] * peak_feature.shape[0],
...
def gene_Peak_Score(pred_label, nodes_id, RNA_matrix, ATAC_matrix, RP_matrix, gene_names):
    ...
    cell_emb_binary = sp.coo_matrix(
        (np.ones(len(y.obs['pred'])), (np.array(range(len(y.obs['pred']))), list(y.obs['pred']))))
    ...
    try:
        ...
        try:
            ...
            for i in range(len(Counter(pred_label))):
                Total_gene_peak_score_df, Total_gene_peak_score = small_gene_Peak_Score(pred_label, RNA_matrix,
                                                                                        ATAC_matrix, RP_matrix,
                                                                                        gene_names, peak_names,
                                                                                        choice_cluser=i)
