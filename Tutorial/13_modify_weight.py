...
def HGT_pre(RNA_matrix, gnn, indices, device, cell_size=30):
    ...
    embedding_dim = (
        gnn.n_hid
    )  # Assuming the hidden dimension is the embedding dimension
    ...
    with torch.no_grad():
        for batch_id in tqdm(range(n_batch), desc="Prediction Batches"):
            ...
            node_type = torch.LongTensor(
                np.array(
                    list(np.zeros(len(cell_index), dtype=int))
                    + list(np.ones(len(gene_index), dtype=int))
                )
            ).to(device)
...
def main():
    ...
    adata_151673 = sc.read_10x_mtx(
        "../data/DLPFC/151673/filtered_feature_bc_matrix",
    ...
    adata_151674 = sc.read_10x_mtx(
        "../data/DLPFC/151672/filtered_feature_bc_matrix",
        var_names="gene_symbols",
        cache=True,
    )
    ...
    ScFormer_result = HGT_pre(
        RNA_matrix, gnn=gnn, indices=indices, device=device, cell_size=cell_size
    )
